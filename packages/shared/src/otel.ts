...
function buildResource(serviceName: string, deploymentEnvironment: string): Resource {
  return new Resource({
    ...
    // Splunk APM maps deployment.environment.name → deployment.environment in UI
      ...
      getNodeAutoInstrumentations({
        ...
        '@opentelemetry/instrumentation-http': {
          ...
          ignoreOutgoingRequestHook: (req) => {
            const path = req.path ?? '';
            const href = `${req.protocol ?? ''}//${req.host ?? req.hostname ?? ''}${path}`;
            return (
              path.includes('/internal/feature-flags') ||
              path.includes('/health') ||
              href.includes('/internal/feature-flags') ||
